...
%Modelo computacional da VBTP GUARANI trafegando por terrenos com variação
%de rugosidade conforme a ISO 8608, como proposto por AGOSTINACCHIO.
...
disp(['Modelo computacional da VBTP GUARANI trafegando por terrenos com variação de rugosidade conforme a ISO 8608, ' ...
...
L = 350;
% Faixa de frequência (m/s-)
dn = 1 / L;
% Frequência Espacial (ciclos/m)
no = 0.1;
% Frequência Espacial Máxima (m/s-)
nmax = 4;
% Número de Pontos
N = nmax / dn;
...
Bd = L / N;
 ...
 %Selecione o valor de classificação de rugosidade ISO 8608
 % Para isso, retire o % de cada K selecionado antes de iniciar.
...
% K=9; %% Classe G-H
 ...
 %------------------MODELO DE PERFIL DE ESTRADA(EQ.42)---------------------
x = 0:Bd:L; % Posição ao longo da estrada
h = zeros(size(x)); % Inicializa o perfil da estrada
fi = 2 * pi * rand(size(x)); % Fase aleatória para o ruído
...
%% Análise da rugosidade com base nos limites do IRI
% Definindo os valores de IRI para diferentes classes (variando K) ISO 8608
iri_values = [1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5]; % Classe A-H

% Verificando se K está dentro do intervalo válido
if K < 2 || K > 9
    error('K deve estar entre 3 e 9');
end

% Definindo o IRI com base no valor de K
iri = iri_values(K - 1); % Subtrai para alinhar com o índice do vetor

if iri < 2.0
    rugosidade_categoria = 'Muito Bom';
elseif iri >= 2.0 && iri < 4.0
    rugosidade_categoria = 'Bom';    
elseif iri >= 4.0 && iri < 6.0
    rugosidade_categoria = 'Média';
elseif iri >= 6.0 && iri < 8.0
    rugosidade_categoria = 'Ruim';   
else
    rugosidade_categoria = 'Muito Ruim';
end

disp(['Categoria da Rugosidade: ', rugosidade_categoria]);
...
% Parâmetros de otimização da suspensão (Rigidez e amortecimento)
...
%% Resultado das respostas dinâmicas 
...
%% Frequência
% Perfil da estrada em função de x
...
% Sistema de Estado para Cada Segmento 
...
fitness_final = w1*fitness_final_a+w2*a_rms_before;
    ...
    figure(81);
    surf(K1, K2, Z); % Plota a superfície
