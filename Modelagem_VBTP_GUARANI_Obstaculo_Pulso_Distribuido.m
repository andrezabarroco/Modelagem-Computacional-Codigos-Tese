...
%pulso distribuído ao longo do trajeto, conforme resolução número 600 do CONTRAN
...
disp(['Modelo computacional da VBTP GUARANI atravessando série de obstáculos distribuído no trajeto conforme a Norma n 600 CONTRAN, ' ...
    'realizando otimização através do algoritmo de PSO'])

%% 1
%% ------------------MODELO DE PERFIL DO OBSTÁCULO----------------------
L = 350; % Comprimento total da estrada em metros
Bd = 0.1; % Passo de discretização em metros
...
% Parâmetros do quebra-molas
comprimento_quebra_molas = 2; % Comprimento do quebra-molas em metros
altura_quebra_molas = 0.1; % Altura do quebra-molas em metros
centro_quebra_molas = 25; % Posição central do quebra-molas em metros
inicio = centro_quebra_molas - comprimento_quebra_molas/2;
fim = centro_quebra_molas + comprimento_quebra_molas/2;
distancia_entre_quebra_molas = 20; % Distância entre os centros de dois quebra-molas consecutivos (metros)
numero_quebra_molas = floor(L/distancia_entre_quebra_molas); % Número de quebra-molas ao longo da estrada

% Geração do perfil do quebra-molas com transição suave
h = zeros(size(x)); % Inicializa o perfil da estrada

for in = 1:numero_quebra_molas
    % Posição central de cada quebra-molas
    centro_quebra_molas = in * distancia_entre_quebra_molas;
    
    % Verificar se o quebra-molas está dentro do intervalo do comprimento da estrada
    if centro_quebra_molas - comprimento_quebra_molas/2 >= 0 && centro_quebra_molas + comprimento_quebra_molas/2 <= L
        % Definir as posições de início e fim do quebra-molas
        inicio = centro_quebra_molas - comprimento_quebra_molas/2;
        fim = centro_quebra_molas + comprimento_quebra_molas/2;

        % Para o intervalo de x que representa o quebra-molas, gerar a forma
        % Transição suave (senoidal ou gaussiana) para o quebra-molas
        idx_quebra_molas = (x >= inicio) & (x <= fim);
        
        % Definir o perfil do quebra-molas com transição suave (usando função seno)
        h(idx_quebra_molas) = altura_quebra_molas * sin(pi * (x(idx_quebra_molas) - inicio) / comprimento_quebra_molas);
    end
end
